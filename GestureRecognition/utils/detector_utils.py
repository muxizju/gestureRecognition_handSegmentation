...
# PATH_TO_LABELS = os.path.join(MODEL_NAME, 'hand_detection.pbtxt')
PATH_TO_LABELS='hand_detection.pbtxt'
...
def draw_box_on_image(num_hands_detect, score_thresh, scores, boxes, im_width, im_height, image_np, classes_str):
    for i in range(num_hands_detect):
        if (scores[i] > score_thresh):
            ...
            res+="   "
            res+=classes_str[i]
            cv2.putText(image_np, res, (int(left/2+right/2),int(0.8*top+0.2*bottom)),
